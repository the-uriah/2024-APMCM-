...
# Function to evaluate the air conditioning position
def evaluate_ac_position(ac_pos):
    T = simulate_temperature(ac_pos)
    # Calculate Mean Squared Error (MSE)
    target_temp = np.full((nx, ny, nz), T_target, dtype=np.float64)
    mse = np.mean((T - target_temp) ** 2)  # Mean Squared Error
    return mse
...
print(f"Best air conditioning position: {best_position}, Mean Squared Error: {best_error}")
